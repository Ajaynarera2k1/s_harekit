...
const fetchFiles = async (session, page=1)=>{
    try {
        let count = 1
        const options = {
            headers: {
        ...
        const res = await axios.get(`/api/file?limit=12&page=${page}`, options)
        const {total, files} = res.data
        const paginationLimit = Math.round(total/12)
        ...
        {
            const pageUi = `
                <button class="rounded px-4 py-2 ${i === page ? 'bg-slate-900 text-white' : 'border-2 border-slate-900 text-slate-900'}" onclick="fetchFiles('${session}', ${i})">${i}</button>
            `
            pages.innerHTML += pageUi
        }
